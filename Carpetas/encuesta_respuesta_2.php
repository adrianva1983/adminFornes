...
$requete = "SELECT MAX(Orden) FROM Publicaciones WHERE IdAmpliacion='".$encuesta."';";
...
{
	for ($i=0;(file_exists ("../../Imagenes/".$NomFoto.$i."p.".$extension));$i++)
	{
	$NomFoto = $NomFoto.$i;
	}
}
...
$requete = "INSERT INTO `Contenidos` (`IdPadre`,`Titulo`, `Tipo`, `Foto`, `AnchoFoto`, `AltoFoto`, `Alternativo`,`Breve`) VALUES ('".$encuesta."', '".$Opcion."', 'encuesta_opcion', '".$NomFoto."', '".$AnchoFoto."', '".$AltoFoto."', '".$Alternativo."', '0');";
mysqli_query($db,$requete);
$requete = "SELECT * FROM `Contenidos` WHERE `Titulo`='".$Opcion."' AND `IdPadre`='".$encuesta."'";

$listado = mysqli_fetch_object($result);
$requete = "INSERT INTO `Publicaciones` (`IdContenido`, `IdAmpliacion`, `Orden`) VALUES ('".$listado->Id."', '".$encuesta."', '".$Orden."');";
mysqli_query($db,$requete);
...
header("Location:../Interface/herramienta.php?modulo=Carpetas&herramienta=encuestas&encuesta=".$encuesta."&seccion=".$seccion."&ruta=".$ruta."&tipocontenido=".$tipocontenido);
?>