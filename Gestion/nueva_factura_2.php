...
$FacturaGasto = $_POST["FacturaGasto"];
...
if ($FacturaGasto!="") $requete.=",`FacturaGasto`";
if ($FacturaGasto==1) $requete.=",`CodigoFacturaGasto`,`IdProveedor`";
...
if ($FacturaGasto==1) 
{
	$requete.=",'".$CodigoFacturaGasto."'";
	if ($IdProveedor!='') $requete.=",".$IdProveedor;
	else $requete.=",NULL";
}
if ($Texto!="") $requete.=",'".$Texto."'";
if ($FormaDePago!="") $requete.=",".$FormaDePago;
...
for($i=0;$i<count($_POST["TextoLinea"]);$i++)
{
	...
	{
		$tempValor = $_POST["BaseImponible"][$i];
