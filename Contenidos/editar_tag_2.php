...
if ($Imagen['tmp_name']!="")
{
	if ($BorrarImagenFichero!="") unlink($_SERVER['DOCUMENT_ROOT'].$BorrarImagenFichero);
...
{
	if ($BorrarImagenFichero!=""&&$BorrarImagen==1) unlink($_SERVER['DOCUMENT_ROOT'].$BorrarImagenFichero);
}
require($_SERVER['DOCUMENT_ROOT']."/administra/Interface/conexion.php");
$requete = "UPDATE `Tags` SET `Nombre`='".$Nombre."'";
if ($Sinonimos!="") $requete.=",`Sinonimos`='".$Sinonimos."'";
else $requete.=",`Sinonimos`=NULL";
if ($IdPadre!="") $requete.=",`IdPadre`=".$IdPadre;
else $requete.=",`IdPadre`=NULL";
if ($URL!="") $requete.=",`URL`='".$URL."'";
else $requete.=",`URL`=NULL";
if ($NomFoto!="") $requete.=",`Imagen`='".$NomFoto."'";
else
{
	if ($BorrarImagen==1) $requete.=",`Imagen`=NULL";
}
$requete.=" WHERE `Id`=".$id.";";
